...
最終的な処方判断は医師が行ってください。
対象年齢: 小児（0〜15歳）。16歳以上には使用しないでください。
特に1歳未満は医師の個別判断が必須です。
...
# =============================================================================
# 症状データベース（4カテゴリ・31症状。初期案26症状 + 追加指示#2で2症状 + 追加指示#6で3症状追加）
# =============================================================================
...
SYMPTOM_CATEGORIES = {
    ...
    "夜泣き・かんしゃく・神経症状": [
        "夜泣き", "かんしゃく・疳の虫", "不眠", "情緒不安定", "熱性けいれん後の体調不良", "不安",
    ],
    "虚弱体質・皮膚・その他": [
        "虚弱体質・疲れやすい", "食が細い", "体重増加不良", "寝汗（盗汗）",
        "アトピー性皮膚炎", "湿疹・おむつかぶれ", "病後の体力低下", "冷え", "ニキビ",
    ],
}
...
SYMPTOM_LIFE_STAGES = {
    ...
    "嘔吐": ["乳児期", "幼児期", "学童期", "思春期"],
    "食欲不振": ["幼児期", "学童期", "思春期"],
    "乳児疝痛（コリック）": ["乳児期"],  # 乳児期特有のため拡張しない
    "便秘・下痢を繰り返す": ["乳児期", "幼児期", "学童期", "思春期"],
    "咳（乾性）": ["乳児期", "幼児期", "学童期", "思春期"],
    "咳（湿性・痰がらみ）": ["乳児期", "幼児期", "学童期", "思春期"],
    "喘息傾向": ["幼児期", "学童期", "思春期"],
    "鼻水・鼻づまり": ["乳児期", "幼児期", "学童期", "思春期"],
    "感冒（かぜ）初期": ["乳児期", "幼児期", "学童期", "思春期"],
    "感冒の遷延・長引く微熱": ["乳児期", "幼児期", "学童期", "思春期"],
    "中耳炎を繰り返す": ["乳児期", "幼児期", "学童期", "思春期"],
    # 追加指示#2で新規追加（暫定・要医学レビュー）
    "咽頭痛・扁桃炎": ["幼児期", "学童期", "思春期"],
    "口内炎": ["乳児期", "幼児期", "学童期", "思春期"],
    "夜泣き": ["乳児期", "幼児期"],  # 乳幼児期特有の訴えのため拡張しない
    "かんしゃく・疳の虫": ["幼児期"],  # 幼児期特有の神経症状のため拡張しない
    "不眠": ["乳児期", "幼児期", "学童期", "思春期"],
    "情緒不安定": ["幼児期", "学童期", "思春期"],
    "熱性けいれん後の体調不良": ["幼児期", "学童期", "思春期"],
    "虚弱体質・疲れやすい": ["幼児期", "学童期", "思春期"],
    "食が細い": ["幼児期", "学童期", "思春期"],
    "体重増加不良": ["乳児期", "幼児期", "学童期", "思春期"],
    "寝汗（盗汗）": ["幼児期", "学童期", "思春期"],
    "アトピー性皮膚炎": ["乳児期", "幼児期", "学童期", "思春期"],
    "湿疹・おむつかぶれ": ["乳児期", "幼児期"],  # おむつかぶれは乳幼児期特有のため拡張しない
    "病後の体力低下": ["幼児期", "学童期", "思春期"],
    # 追加指示#6で新規追加（暫定・要医学レビュー）
    "冷え": ["幼児期", "学童期", "思春期"],
    "ニキビ": ["学童期", "思春期"],  # 思春期のホルモン変化に伴う尋常性ざ瘡を主眼、学童期後期も含む
    "不安": ["幼児期", "学童期", "思春期"],  # 言語化された不安の訴えは幼児期以降を想定、乳児期は対象外
}


# =============================================================================
# 漢方薬データベース（保険適用漢方 - ツムラ番号は暫定値・要添付文書検証）
# 小児科向け29処方（初期案19処方 + 追加指示#2で8処方 + 追加指示#6で2処方追加）
#
...
KAMPO_DATABASE = {
    "小建中湯": {
        ...
        "notes": {
            ...
            "age_7_9": "成人1日量の約3/5を目安。体質改善に。",
            "age_10_12": "成人1日量の約7/10を目安。体質改善に。",
            "age_13_15": "成人1日量の約85%を目安。体質改善に。",
            "caution": "膠飴含有で甘く比較的服用しやすい。嫌がる場合は服薬ゼリーやお湯で溶いて冷まして与える。",
        },
        "life_stage_eligible": ["乳児期", "幼児期", "学童期", "思春期"],
    },
    "黄耆建中湯": {
        "products": {
        ...
        "notes": {
            ...
            "age_6": "成人1日量の約1/2を目安。",
            "age_7_9": "成人1日量の約3/5を目安。",
            "age_10_12": "成人1日量の約7/10を目安。",
            "age_13_15": "成人1日量の約85%を目安。",
            "caution": "苦味は強くないが、服薬を嫌がる場合は服薬ゼリー等を検討。",
        },
        "life_stage_eligible": ["幼児期", "学童期", "思春期"],
    },
    "六君子湯": {
        "products": {
    ...
    "五虎湯": {
        ...
        "notes": {
            ...
            "age_7_9": "成人1日量の約3/5を目安。慎重投与。",
            "age_10_12": "成人1日量の約7/10を目安。慎重投与。",
            "age_13_15": "成人1日量の約85%を目安。慎重投与。",
    ...
    "甘麦大棗湯": {
        ...
        "notes": {
            ...
            "age_7_9": "成人1日量の約3/5を目安。甘草量に留意。",
            "age_10_12": "成人1日量の約7/10を目安。甘草量に留意。",
            "age_13_15": "成人1日量の約85%を目安。甘草量に留意。",
    ...
    "麻黄湯": {
        ...
        "notes": {
            ...
            "age_7_9": "成人1日量の約3/5を目安。短期間の使用に限る。",
            "age_10_12": "成人1日量の約7/10を目安。短期間の使用に限る。",
            "age_13_15": "成人1日量の約85%を目安。短期間の使用に限る。",
    ...
    "芍薬甘草湯": {
        ...
        "notes": {
            ...
            "age_7_9": "成人1日量の約3/5を目安に頓用。連用しない。",
            "age_10_12": "成人1日量の約7/10を目安に頓用。連用しない。",
            "age_13_15": "成人1日量の約85%を目安に頓用。連用しない。",
    ...
    # =========================================================================
    # 追加指示#6（2026-08-12）で追加された2処方
    # ツムラ番号・生薬量・エビデンス・life_stage_eligible は暫定値。
    # 実運用前に添付文書・インタビューフォームでの検証が必須。
    # =========================================================================
    "当帰四逆湯": {
        "products": {
            "ツムラ": {"product_no": 38, "available": True},
            "クラシエ": {"product_no": None, "available": True},  # 暫定: 製品番号未確認・取扱有無要確認（要IF検証）
            "コタロー": {"product_no": None, "available": True},  # 暫定: 製品番号未確認・取扱有無要確認（要IF検証）
        },
        "insurance": True,
        "sho": "虚証",
        "description": "四肢末梢の冷え・しもやけ傾向に。虚証で血行改善を図る代表方",
        "indications": {
            "primary": ["冷え"],
            "secondary": ["虚弱体質・疲れやすい"],
            "off_label": [],
        },
        "evidence": {
            "level": "C",
            "guideline_grade": "2B",
            "references": [
                "ツムラ当帰四逆湯エキス顆粒 添付文書・インタビューフォーム",
                "日東医誌: 冷え性・末梢循環への使用報告",
            ],
            "efficacy_rate": 0.50,  # 暫定値
            "clinical_usage_rank": 35,
        },
        "onset": {
            "initial": "1-2週間",
            "optimal": "4-8週間",
        ...
        "contraindications": ["著明な実証", "下痢を伴う急性期"],
        "precautions": ["甘草含有製剤併用時", "のぼせやすい児"],
        "combinations": {
            "recommended": ["人参湯"],
            "possible": ["五積散"],
        ...
        "side_effects": {
            ...
            "rare": ["偽アルドステロン症", "発疹"],
            "serious": ["偽アルドステロン症（稀）"],
            "note": "甘草含有。麻黄・大黄・附子は含まない想定（要IF検証）",
        },
        "notes": {
            "age_under_1": "医師相談必須。乳児期での使用実績は特に限定的。原則候補外。",
            "age_1_2": "成人1日量の約1/3を目安。冷えが強い虚証の場合に医師判断で検討。",
            "age_3_5": "成人1日量の約1/2を目安。四肢冷え・しもやけ傾向に用いることがある。",
            "age_6": "成人1日量の約1/2を目安。",
            "age_7_9": "成人1日量の約3/5を目安。学童期の冷え性・末梢循環改善に。",
            "age_10_12": "成人1日量の約7/10を目安。",
            "age_13_15": "成人1日量の約85%を目安。思春期の冷えにも用いることがある。",
            "caution": "甘味があり比較的服用しやすい。服薬を嫌がる場合は服薬ゼリー等を検討。",
        },
        # 当帰四逆湯は幼児期でも実質使用されにくいが、冷え症状は学童期以降が主眼のため学童期から
        "life_stage_eligible": ["学童期", "思春期"],
    },
    "清上防風湯": {
        "products": {
            "ツムラ": {"product_no": 58, "available": True},
            "クラシエ": {"product_no": None, "available": True},  # 暫定: 製品番号未確認・取扱有無要確認（要IF検証）
            "コタロー": {"product_no": None, "available": True},  # 暫定: 製品番号未確認・取扱有無要確認（要IF検証）
        },
        "insurance": True,
        "sho": "実証〜中間証",
        "description": "思春期の尋常性ざ瘡（ニキビ）に。上部（顔面）の熱毒・血行鬱滞を改善",
        "indications": {
            "primary": ["ニキビ"],
            "secondary": ["アトピー性皮膚炎"],
            "off_label": [],
        ...
        "contraindications": ["著明な虚証", "下痢を伴う場合"],
        "precautions": ["甘草含有製剤併用時", "長期連用時は肝機能に注意"],
        "combinations": {
            "recommended": ["温清飲"],
            "possible": ["消風散"],
            "avoid": [],
        ...
        "notes": {
            "age_under_1": "医師相談必須。乳児期での使用実績は特に限定的。原則候補外。",
            "age_1_2": "原則候補外。ニキビは学童期以降の適応を想定。",
            "age_3_5": "原則候補外。ニキビは学童期以降の適応を想定。",
            "age_6": "原則候補外。ニキビは学童期以降の適応を想定。",
            "age_7_9": "成人1日量の約3/5を目安。学童期後期のニキビに用いることがある。",
            "age_10_12": "成人1日量の約7/10を目安。思春期前後の尋常性ざ瘡に。",
            "age_13_15": "成人1日量の約85%を目安。思春期のホルモン性ざ瘡に。",
            "caution": "苦味があり嫌がる場合がある。服薬ゼリー等の活用を検討。",
        },
        # notes（age_1_2/age_3_5/age_6は「原則候補外」）と整合させるため幼児期は含めない
        "life_stage_eligible": ["学童期", "思春期"],
    },
...
SYMPTOM_KAMPO_WEIGHTS = {
    ...
    # 追加指示#6で新規追加（暫定・要医学レビュー）
    "冷え": {
        "当帰四逆湯": {"weight": 5, "evidence_level": "C", "recommendation_grade": "2B"},
        "五積散": {"weight": 3, "evidence_level": "C", "recommendation_grade": "2C"},
        "人参湯": {"weight": 3, "evidence_level": "C", "recommendation_grade": "2C"},
    },
    "ニキビ": {
        "清上防風湯": {"weight": 5, "evidence_level": "C", "recommendation_grade": "2B"},
        "温清飲": {"weight": 4, "evidence_level": "C", "recommendation_grade": "2B"},
        "消風散": {"weight": 3, "evidence_level": "C", "recommendation_grade": "2C"},
    },
    "不安": {
        "抑肝散": {"weight": 5, "evidence_level": "B", "recommendation_grade": "1B"},
        "甘麦大棗湯": {"weight": 4, "evidence_level": "C", "recommendation_grade": "2B"},
...
HERB_COMPONENTS = {
    ...
    # 追加指示#6で追加された2処方（暫定値・要IF検証）
    "当帰四逆湯": {
        "ツムラ": {"甘草": 2.0, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: 麻黄・大黄・附子非含有想定・要IF検証
        "クラシエ": {"甘草": 2.0, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: ツムラと同値と仮定・要IF検証
        "コタロー": {"甘草": 2.0, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: ツムラと同値と仮定・要IF検証
    },
    "清上防風湯": {
        "ツムラ": {"甘草": 1.5, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: 麻黄・大黄・附子非含有想定・要IF検証
        "クラシエ": {"甘草": 1.5, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: ツムラと同値と仮定・要IF検証
        "コタロー": {"甘草": 1.5, "麻黄": 0, "大黄": 0, "附子": 0},  # 暫定: ツムラと同値と仮定・要IF検証
    },
}
...
COMBINATION_PATTERNS = [
    ...
    },    # 追加指示#6で追加された併用パターン（甘草量を check_combination_safety() で確認済み）
    {
        "combination": ["当帰四逆湯", "人参湯"],
        "indication": "冷えを伴う虚弱体質・疲労感",
        "note": "当帰四逆湯（末梢冷え）に人参湯（気虚）を併用",
    },
    {
        "combination": ["清上防風湯", "温清飲"],
        "indication": "思春期のニキビに皮膚の乾燥・かゆみを伴う",
        "note": "清上防風湯（上部熱毒）に温清飲（血熱・皮膚症状）を併用",
    },
]
